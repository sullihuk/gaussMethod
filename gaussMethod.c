...
void reverseTriangle(float system[row][row+1] ) //Метод получает на вход матрицу и преобразует ее к т.н. треугольному (сверху) виду
{
  for (int t = 0; t<row; t++)
  {
    for(int i = 0; i<row; i++) 
    {
      ...
      {	
        printf("%.2f\t",	system[i][j] ); // В теле цикла назначенное значение переназначается. В данном случае это значение является вспомогательным.
      }	
      puts("");
    }
      puts("===================");
